...
* CloudHSMを利用することでハードウェア暗号化を実施できる
...
* vCPU制限があり、それ以上の起動数は確保できない
* ステップスケーリングポリシーではウォームアップが設定可能
...
* 別のAWSアカウントに所有するS3バケットの利用を許可したい場合、そのアカウントとユーザに対してIAMポリシーとバケットポリシー両方の設定許可が必要
* MAX 5TB
* S3アクセスポイントを設定することによりデータアクセスの管理を簡素化する
* パブリック・アクセス設定機能のよりパブリック・アクセスをブロック、許可できる
* AWS Lake Formationによりデータレイク構成を容易に実施できる
* Transfer Accelerationはクライアントへの長距離に渡るファイル転送を高速に安全に実施できる
* CORSを設定するとすでにドメインが設定されているS3バケットを他のドメインの共有することができる
# Amazon Glacier 
* Flexible Retrieval
...
* プロビジョンドIOPSはマルチインスタンスにあっちで切る
* 外部サービスを呼び出すときはIAMロールをEC2インスタンスではなくECSタスクに付与する
...
* インスタンスストアがデフォルトで利用される.一時的なデータが保持される
...
* スポットフリートはインスタンスタイプや入札価格を指定して自動的に最安値のインスタンスを選択してスポットインスタンス数を調整してリクエストを処理する
* EC2フリーとはオンデマンドとスポットで構成されるインスタンスグループとして設定を定義する
* クラスタプレイスメントグループにより単一のAZ内のインスタンスのパフォーマンスを向上させるためにグループ化する
  * T3は利用できない,M5をつかう
## EBSボリューム
...
* Amazon Data Lifecycle Manager(DLM)
  ...
  * コンプラ向けのバックアップ
  * 不要なバックアップを削除
  * バックアップ対象はEBSボリュームとスナップショットの提示取得や管理のみ
...
* ACMがサポートsれていないリージョンではIAMをSSL証明書マネージャーとして使用する
...
* トラフィックフローを用いて複雑なルーティングをせっていできる
* CNAMEは特定のホストの別のドメイン名に転送するときに使う,エイリアスではない
...
* ライフサイクルポリシーによるバックアップはない。自動バックアップ取得を使う
* シャーディングによりデータを複数のDBに振り分けてパフォーマンスを上げる
...
* AWS Transit Gatewayはネットワークハブとなり、VPC間の接続をハブ＆スポークスにする
...
* Egress trueでアウトバンド、falseでインバウンドの設定
...
# Storage Gateway
* デフォルトでS3を使用する
* AWS BackupによりAWSサービス全体のデータのバックアップの一元化と自動化をかんたんに実行できる

# VPN
* オンプレ側にはカスタマーゲートウェイを置く
* サイト間VPNでオンプレとVPCをつなぐ
* VPCエンドポイントはVPC間をつなぐもの
* Acceleratedサイト間VPNにより高可用性、高パフォーマンスが維持できる
...
* AWS暗号化SDKライブラリを使用すると暗号化のベストプラクティスによる暗号化をかんたんに実装できる
* IAM Acceess AnalyzerによりS3バケットやIAMロールを分析してセキュリティリスクのリソースとデータへの意図しないアクセスを特定できる
# EFS
* オンプレからの移行の際にはオンプレにAWS DataSyncエージェントをインストールする
  * Datasyncは簡素化、自動化、高速化されている
* 利用するEC2インスタンスは同じVPCが望ましい
* バーストスループットモードを使うとスループットがファイルシステムのサイズに合わせてスケールされる
* 汎用モードは一般用途を想定、レイテンシーが最も低い
* 最大I/Oモードはクライアントからの同時アクセスを想定、合計スループットを優先する。レイテンシーが多少長くなる
* ライフサイクルを有効にすることにより自動的かつ透過的にEFS IAなどにファイルが移動される
* NFSv4プロトコルを使う
...
* EC2の起動中はコストが発生
* 停止中のEBSにコストが発生
* AWS Organizationsを利用して一括請求を設定するとストレージを統合してS3コストを下げられる
* リザーブドできるサービスは
  * DynamoDBリザーブドキャパシティ
  * Redshiftリザーブドノード
  * ElastiCacheリザーブドキャッシュノード
* Savings Plansは一定量、一定期間の利用に適用される
...
# Amazon EMR
* 大規模環境で大量のデータを迅速かつコスト効率よく処理できるビッグデータ所利用プラットフォーム
* ユースケースは
  * MachineLearning
  * クリックストリーム分析
  * リアルタイムストリーミングデータ処理
